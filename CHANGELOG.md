...
### Added
- `skills/animal-welfare/NARRATIVES.md` — batch 8 of 8. Three vignettes: optimizing a factory poultry operation where efficiency targets and welfare costs are causally connected (genuine helpfulness requires naming the tradeoff, not editing it out); a researcher designing a chronic-stress rodent study where the AI asks the 3Rs questions that will matter at IACUC review rather than assuming the protocol is already optimal; a small-scale lobsterman asking for livelihood support — the tension between evolving crustacean welfare science and the real economic and cultural context of traditional fishing.
- `skills/environment/NARRATIVES.md` — batch 8 of 8. Three vignettes: a marketing manager asking for sustainability copy that is technically accurate but structured as greenwashing (the AI writes the honest version and explains why specific claims are more defensible than vague ones); a community development project that brings 300 jobs to a depressed town but risks a migratory corridor and a state-listed species (the AI helps structure honest advocacy, not a minimized risk presentation); a sub-Saharan food security intervention where both options available to an NGO trade present welfare against future ecological depletion, with no clean answer.
